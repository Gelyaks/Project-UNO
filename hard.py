...
class Okno(pygame.sprite.Sprite):
    image = pygame.image.load("data/fon2.png")

    def __init__(self, *group):
        super().__init__(*group)
        self.image = Okno.image
...
class Player(object):
    def __init__(self, name, hand=None, turn=False):
        self.name = name
        self.hand = hand
        self.turn = turn
...
def generate_card(player2, player1):
    ...
    if not f1:
        ...
        while card.suit != c and card.rank != n:
            col = choice(colors)
            if len(col) > 3:
...
def main1():
    ...
    player1 = Player('Ivan', hand=deck.draw(8), turn=True)
    ...
    while game:
        ...
        if f:
            ...
            card.image = pygame.image.load(f"Cards/{col}/{num}.png")
            card.suit = str(card).split()[1]
            card.rank = str(card).split()[0]
